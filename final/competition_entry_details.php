...
$stmt = $conn->prepare("SELECT competition_entries.title, competition_entries.description, competition_entries.submission, 
...
$stmt = $conn->prepare("SELECT end_date FROM Competitions WHERE id = ?");
...
if (isset($_SESSION['user_id'])) {
    $stmt = $conn->prepare("SELECT id FROM likes WHERE user_id = ? AND entry_id = ?");
...
$stmt = $conn->prepare("SELECT competition_entries_comments.id, competition_entries_comments.content, users.username, competition_entries_comments.created_at
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($content)) {
        if ($commentId) {
            ...
            $stmt = $conn->prepare("UPDATE competition_entries_comments SET content = ? WHERE id = ? AND user_id = ?");
            $stmt->bind_param("sii", $content, $commentId, $userId);
        } else {
            // Insert new comment
            $stmt = $conn->prepare("INSERT INTO competition_entries_comments (entry_id, user_id, content) VALUES (?, ?, ?)");
