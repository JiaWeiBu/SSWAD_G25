...
$stmt = $conn->prepare("SELECT competition_entries.id AS entry_id, competition_entries.title, competition_entries.description, 
...
<div class="container mt-3">
    ...
    <?php if ($isOngoing || (isset($_SESSION['user_id']) && $_SESSION['user_id'] === $competition['user_id'])): ?>
        <div class="text-end mb-3 d-flex justify-content-end gap-2">
            <?php if ($isOngoing): ?>
                ...
                <?php endif; ?>
            <?php endif; ?>
            <?php if (isset($_SESSION['user_id']) && $_SESSION['user_id'] === $competition['user_id']): ?>
...
<script>
    ...
    function deleteCompetition(competitionId) {
        if (confirm('Are you sure you want to delete this competition? This action cannot be undone.')) {
            const xhr = new XMLHttpRequest();
            xhr.open('POST', 'competition_delete.php', true); // Updated file reference
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xhr.onload = function () {
                if (xhr.status === 200) {
