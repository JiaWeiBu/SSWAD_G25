...
$stmt = $conn->prepare("SELECT id AS recipe_id, name AS title FROM recipes WHERE user_id = ?"); // Changed created_by to user_id
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($description) || empty($submission)) {
        ...
        $stmt = $conn->prepare("INSERT INTO competition_entries (competition_id, user_id, recipe_id, title, description, submission) VALUES (?, ?, ?, ?, ?, ?)");
...
<div class="container mt-3">
    ...
    <?php elseif (empty($recipes)): ?>
        ...
        <a href="add_recipe.php" class="btn btn-primary">Create Recipe</a>
    <?php else: ?>
        <form method="post">
            <div class="mb-3">
