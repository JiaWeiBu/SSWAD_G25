...
// Fetch entry details
$stmt = $conn->prepare("SELECT title, description, submission, recipe_id, user_id FROM competition_entries WHERE id = ?");
...
if (!isset($_SESSION['user_id']) || (int)$_SESSION['user_id'] !== (int)$entryDetails['user_id']) {
...
$stmt = $conn->prepare("SELECT id AS recipe_id, name AS title FROM recipes WHERE user_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($description) || empty($submission) || empty($recipeId)) {
        ...
        if (!$recipe) {
            ...
            // Update the entry
            $stmt = $conn->prepare("UPDATE competition_entries SET title = ?, description = ?, submission = ?, recipe_id = ? WHERE id = ?");
