...
if ($competition_id) {
    $stmt = $conn->prepare("SELECT * FROM Competitions WHERE id = ? AND user_id = ?"); // Changed created_by to user_id
    ...
    if ($current_user_id) { // Ensure user is logged in
        ...
        if (empty($title) || empty($description) || empty($start_date) || empty($end_date)) {
            ...
            if ($start_date < $current_date) { // Allow today as a valid start date
                ...
                $stmt = $conn->prepare("UPDATE Competitions SET title = ?, description = ?, start_date = ?, end_date = ? WHERE id = ? AND user_id = ?"); // Changed created_by to user_id
